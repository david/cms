defmodule CMSWeb.UserAuthTest do
  ...
  @remember_me_cookie "_cms_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, CMSWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      CMSWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_scope_for_user/2" do
    test "authenticates user from session", %{conn: conn, user: user} do
      ...
      assert conn.assigns.current_scope.user.organization_id == user.id
  ...
  describe "on_mount :mount_current_scope" do
    ...
    test "assigns current_scope based on a valid user_token", %{conn: conn, user: user} do
      ...
      assert updated_socket.assigns.current_scope.user.organization_id == user.id
    end
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        CMSWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
